  ...
  return (
    <group position={[centerX, centerY, centerZ]} rotation={[0, rotateY, 0]}>
      ...
      <mesh receiveShadow position={[-width / 4, 0, 0.02]}>
        ...
        <meshPhysicalMaterial color="#d6f0ff" transmission={0.95} opacity={1} transparent roughness={0.02} metalness={0} thickness={0.2} ior={1.45} reflectivity={0.9} clearcoat={1} clearcoatRoughness={0} />
      </mesh>
      {/* Right glass pane */}
      <mesh receiveShadow position={[width / 4, 0, 0.02]}>
        <boxGeometry args={[width / 2 - frame * 0.5, height, Math.max(0.02, depth * 0.22)]} />
...
function createPlanningId(prefix) {
  return typeof crypto !== "undefined" && typeof crypto.randomUUID === "function"
    ? crypto.randomUUID()
    : `${prefix}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
}

function normalizePlanningCategory(value, fallback = "office") {
  const raw = String(value || "").trim().toLowerCase();
  if (!raw) return fallback;
  if (["2bhk", "1bhk", "studio", "apartment", "bedroom", "luxury bedroom", "house", "home", "flat"].includes(raw)) return "house";
  if (raw.includes("cafe") || raw.includes("coffee")) return "cafe";
  if (raw.includes("office") || raw.includes("workspace")) return "office";
  if (raw.includes("toilet") || raw.includes("restroom") || raw.includes("washroom")) return "public toilet";
  if (raw.includes("security") || raw.includes("guard")) return "security cabin";
  if (raw.includes("storage") || raw.includes("warehouse")) return "storage";
  return PRODUCT_CATEGORIES.includes(raw) ? raw : fallback;
}

function inferCategoryFromPrompt(prompt, fallback = "office") {
  const lower = String(prompt || "").toLowerCase();
  if (!lower.trim()) return fallback;
  if (/(2\s*bhk|two bedroom|family home|apartment|flat|house|home|studio|bedroom|luxury bedroom)/.test(lower)) return "house";
  if (/(cafe|coffee|container cafe|restaurant|counter seating)/.test(lower)) return "cafe";
  if (/(office|workspace|workstations?|desks?|meeting room|reception)/.test(lower)) return "office";
  if (/(public toilet|restroom|washroom|urinal|stalls?|basins?)/.test(lower)) return "public toilet";
  if (/(security cabin|guard room|guard cabin|cctv)/.test(lower)) return "security cabin";
  if (/(storage|warehouse|rack|inventory)/.test(lower)) return "storage";
  return fallback;
}

function getPromptNumber(prompt, patterns, fallback = 0) {
  const lower = String(prompt || "").toLowerCase();
...
function getDefaultPlanDimensionsByCategory(category, prompt = "", currentState = {}) {
  const extracted = extractPlanDimensions(prompt) || {};
  const normalizedCategory = normalizePlanningCategory(category, normalizePlanningCategory(currentState?.selectedCategory, "office"));
  const defaults = {
    house: { totalWidth: 40, totalHeight: 30 },
    cafe: { totalWidth: 32, totalHeight: 24 },
    office: { totalWidth: 42, totalHeight: 28 },
    "public toilet": { totalWidth: 24, totalHeight: 16 },
    "security cabin": { totalWidth: 12, totalHeight: 10 },
    storage: { totalWidth: 50, totalHeight: 36 },
  };
  const base = defaults[normalizedCategory] || defaults.office;
  return {
    totalWidth: Number(extracted.totalWidth) || Number(currentState?.totalWidth) || base.totalWidth,
    totalHeight: Number(extracted.totalHeight) || Number(currentState?.totalHeight) || base.totalHeight,
  };
}

function getRoomTypeFromName(name, category = "office") {
  ...
  if (category === "house") {
    ...
    if (lower.includes("bath") || lower.includes("toilet") || lower.includes("wash")) return "bathroom";
    if (lower.includes("dining")) return "dining";
    if (lower.includes("bed") || lower.includes("master") || lower.includes("guest")) return "bedroom";
  }
  if (category === "office") {
    ...
    if (lower.includes("cabinet") || lower.includes("storage")) return "storage";
    return "workspace";
  }
  if (category === "cafe") {
    if (lower.includes("counter") || lower.includes("cash")) return "counter";
    if (lower.includes("kitchen") || lower.includes("prep")) return "prep";
    if (lower.includes("display")) return "display";
    return "seating";
  }
  if (category === "public toilet") {
    if (lower.includes("wash")) return "wash";
    if (lower.includes("male") || lower.includes("female") || lower.includes("stall") || lower.includes("wc")) return "stalls";
    return "toilet";
  }
  if (category === "security cabin") return "cabin";
  if (category === "storage") return "storage";
  return lower.includes("bath") ? "bathroom" : "room";
}

function getMinimumRoomSize(type, category) {
  const key = `${category}:${type}`;
  const map = {
    "house:living": { width: 12, height: 10 },
    "house:bedroom": { width: 10, height: 10 },
    "house:kitchen": { width: 8, height: 6 },
    "house:bathroom": { width: 5, height: 6 },
    "house:dining": { width: 8, height: 7 },
    "office:workspace": { width: 14, height: 12 },
    "office:reception": { width: 8, height: 8 },
    "office:meeting": { width: 9, height: 8 },
    "office:pantry": { width: 6, height: 5 },
    "cafe:seating": { width: 14, height: 12 },
    "cafe:counter": { width: 8, height: 6 },
    "cafe:prep": { width: 8, height: 6 },
    "public toilet:stalls": { width: 8, height: 8 },
    "public toilet:wash": { width: 6, height: 4 },
    "security cabin:cabin": { width: 8, height: 8 },
    "storage:storage": { width: 18, height: 14 },
  };
  return map[key] || { width: 8, height: 8 };
}

function buildPlanningRoom(index, name, type, width, height, category, overrides = {}) {
  return createTemplateRoom(index, name, width, height, category, { type, ...overrides });
}

function getHouseProgram(prompt, totalWidth, totalHeight) {
  const lower = String(prompt || "").toLowerCase();
  const wantsStudio = /studio/.test(lower);
  const wantsOneBed = /(^|\s)(1\s*bhk|one bedroom)(\s|$)/.test(lower);
  const bathCount = /two bathrooms|2 bathrooms|2 bath/.test(lower) || totalWidth >= 36 ? 2 : 1;
  if (wantsStudio) {
    return [
      { name: "Studio Living", type: "living", width: Math.max(16, totalWidth * 0.62), height: Math.max(12, totalHeight * 0.68) },
      { name: "Kitchen", type: "kitchen", width: Math.max(8, totalWidth * 0.22), height: Math.max(7, totalHeight * 0.32) },
      { name: "Bathroom", type: "bathroom", width: 6, height: 7 },
    ];
  }
  if (wantsOneBed) {
    return [
      { name: "Living Room", type: "living", width: 14, height: 11 },
      { name: "Bedroom 1", type: "bedroom", width: 12, height: 11 },
      { name: "Kitchen", type: "kitchen", width: 9, height: 7 },
      { name: "Bathroom", type: "bathroom", width: 6, height: 7 },
    ];
  }
  return [
    { name: "Living Room", type: "living", width: 14, height: 12 },
    { name: "Master Bedroom", type: "bedroom", width: 12, height: 11 },
    { name: "Bedroom 2", type: "bedroom", width: 11, height: 10 },
    { name: "Kitchen", type: "kitchen", width: 10, height: 8 },
    ...Array.from({ length: bathCount }, (_, index) => ({ name: `Bathroom ${index + 1}`, type: "bathroom", width: 6, height: 7 })),
  ];
}

function generateCategorySeedLayout(category, totalWidth, totalHeight, prompt = "", variantStyle = "standard") {
  const normalizedCategory = normalizePlanningCategory(category, inferCategoryFromPrompt(prompt, "office"));
  const lower = String(prompt || "").toLowerCase();
  const seatCount = getPromptNumber(prompt, [/for\s+(\d+)\s+people/, /(\d+)\s+seats?/, /(\d+)\s+chairs?/], normalizedCategory === "cafe" ? 12 : 0);
  const deskCount = getPromptNumber(prompt, [/(\d+)\s+workstations?/, /(\d+)\s+desks?/, /office\s+for\s+(\d+)\s+people/], normalizedCategory === "office" ? 10 : 0);
  const wantsMeetingRoom = /meeting room|conference/.test(lower);
  const wantsReception = /reception/.test(lower) || normalizedCategory === "office";
  const wantsDisplay = /display/.test(lower) || normalizedCategory === "cafe";
  const wantsMoreWindows = /airy|sunlight|windows|premium|luxury/.test(lower);

  let rooms = [];
  let planName = `${getFriendlyCategoryName(normalizedCategory)} Layout`;

  if (normalizedCategory === "house") {
    planName = /studio/.test(lower) ? "Studio Apartment Layout" : /1\s*bhk|one bedroom/.test(lower) ? "1BHK Floor Plan" : "2BHK Floor Plan";
    rooms = getHouseProgram(prompt, totalWidth, totalHeight).map((room, index) => buildPlanningRoom(index, room.name, room.type, room.width, room.height, "house"));
  } else if (normalizedCategory === "office") {
    rooms = [
      ...(wantsReception ? [buildPlanningRoom(0, "Reception", "reception", 10, 8, "office")] : []),
      buildPlanningRoom(1, "Workspace", "workspace", Math.max(16, 12 + Math.ceil(deskCount / 2) * 2), Math.max(12, 10 + Math.ceil(deskCount / 4) * 2), "office"),
      ...(wantsMeetingRoom || deskCount >= 8 ? [buildPlanningRoom(2, "Meeting Room", "meeting", 10, 8, "office")] : []),
      ...(deskCount >= 12 ? [buildPlanningRoom(3, "Storage", "storage", 8, 6, "office")] : []),
    ];
  } else if (normalizedCategory === "cafe") {
    rooms = [
      buildPlanningRoom(0, "Seating Area", "seating", Math.max(15, 12 + Math.ceil(seatCount / 6) * 2), Math.max(12, 10 + Math.ceil(seatCount / 8) * 2), "cafe"),
      buildPlanningRoom(1, "Service Counter", "counter", 9, 6, "cafe"),
      buildPlanningRoom(2, "Kitchen / Prep", "prep", 9, 7, "cafe"),
      ...(wantsDisplay ? [buildPlanningRoom(3, "Display Nook", "display", 6, 5, "cafe")] : []),
    ];
    planName = "Cafe Layout";
  } else if (normalizedCategory === "public toilet") {
    const stallCount = Math.max(2, getPromptNumber(prompt, [/(\d+)\s+stalls?/, /(\d+)\s+wc/], 4));
    rooms = [
      buildPlanningRoom(0, stallCount >= 4 ? "Stall Area" : "Toilet Area", "stalls", Math.max(10, 6 + stallCount * 1.5), 9, "public toilet"),
      buildPlanningRoom(1, "Wash Area", "wash", 8, 5, "public toilet"),
      ...(stallCount >= 4 ? [buildPlanningRoom(2, "Urinal / Basin Side", "toilet", 8, 6, "public toilet")] : []),
    ];
    planName = "Public Toilet Layout";
  } else if (normalizedCategory === "security cabin") {
    rooms = [buildPlanningRoom(0, "Security Cabin", "cabin", Math.max(8, totalWidth - 2), Math.max(8, totalHeight - 2), "security cabin")];
    planName = "Security Cabin Layout";
  } else {
    rooms = [buildPlanningRoom(0, "Storage Area", "storage", Math.max(18, totalWidth - 4), Math.max(14, totalHeight - 4), "storage")];
    planName = "Storage Layout";
  }

  const styleScale = variantStyle === "compact" ? 0.92 : variantStyle === "airy" ? 1.06 : 1;
  const scaledRooms = rooms.map((room) => {
    const min = getMinimumRoomSize(room.type || getRoomTypeFromName(room.name, normalizedCategory), normalizedCategory);
    let nextWidth = Math.max(min.width, Math.round(Number(room.width) * styleScale));
    let nextHeight = Math.max(min.height, Math.round(Number(room.height) * styleScale));
    if (variantStyle === "airy" && ["living", "workspace", "seating"].includes(room.type)) {
      nextWidth += 2; nextHeight += 1;
    }
    if (variantStyle === "compact" && ["bathroom", "wash", "storage", "counter"].includes(room.type)) {
      nextWidth = Math.max(min.width, nextWidth - 1);
    }
    return { ...room, width: nextWidth, height: nextHeight, wantsMoreWindows };
  });
  ...
  return {
    ...
    totalWidth,
    totalHeight,
    responseText: `Created a practical ${planName.toLowerCase()} from your description.`,
    rooms: scaledRooms,
  };
}

function getSharedBoundary(roomA, roomB) {
  const aLeft = Number(roomA.x) || 0;
  const aTop = Number(roomA.y) || 0;
  const aRight = aLeft + (Number(roomA.width) || 0);
  const aBottom = aTop + (Number(roomA.height) || 0);
  const bLeft = Number(roomB.x) || 0;
  const bTop = Number(roomB.y) || 0;
  const bRight = bLeft + (Number(roomB.width) || 0);
  const bBottom = bTop + (Number(roomB.height) || 0);

  if (Math.abs(aRight - bLeft) < 0.05 && rangesOverlapInclusive(aTop, aBottom, bTop, bBottom)) {
    return { wallA: "right", wallB: "left", start: Math.max(aTop, bTop), end: Math.min(aBottom, bBottom) };
  }
  if (Math.abs(aLeft - bRight) < 0.05 && rangesOverlapInclusive(aTop, aBottom, bTop, bBottom)) {
    return { wallA: "left", wallB: "right", start: Math.max(aTop, bTop), end: Math.min(aBottom, bBottom) };
  }
  if (Math.abs(aBottom - bTop) < 0.05 && rangesOverlapInclusive(aLeft, aRight, bLeft, bRight)) {
    return { wallA: "bottom", wallB: "top", start: Math.max(aLeft, bLeft), end: Math.min(aRight, bRight) };
  }
  if (Math.abs(aTop - bBottom) < 0.05 && rangesOverlapInclusive(aLeft, aRight, bLeft, bRight)) {
    return { wallA: "top", wallB: "bottom", start: Math.max(aLeft, bLeft), end: Math.min(aRight, bRight) };
  }
  return null;
}

function placeRoomsByCategory(rooms, category, totalWidth, totalHeight) {
  const normalizedCategory = normalizePlanningCategory(category, "office");
  const typed = (Array.isArray(rooms) ? rooms : []).map((room) => ({ ...room, type: room.type || getRoomTypeFromName(room.name, normalizedCategory) }));
  if (!typed.length) return [];

  if (normalizedCategory === "security cabin" || normalizedCategory === "storage") {
    return typed.map((room, index) => ({
      ...room,
      x: index === 0 ? 0.8 : FURNITURE_WALL_CLEARANCE,
      y: index === 0 ? 0.8 : FURNITURE_WALL_CLEARANCE,
      width: Math.min(Number(room.width) || totalWidth - 1.6, totalWidth - 1.6),
      height: Math.min(Number(room.height) || totalHeight - 1.6, totalHeight - 1.6),
    }));
  }

  if (normalizedCategory === "house") {
    const living = typed.find((room) => room.type === "living") || typed[0];
    const kitchen = typed.find((room) => room.type === "kitchen");
    const dining = typed.find((room) => room.type === "dining");
    const bathrooms = typed.filter((room) => room.type === "bathroom");
    const bedrooms = typed.filter((room) => room.type === "bedroom");
    const layout = [];
    const frontBand = Math.max(10, Math.round(totalHeight * 0.4));
    const livingWidth = Math.max(12, Math.round(totalWidth * 0.45));
    if (living) layout.push({ ...living, x: 0, y: 0, width: Math.min(livingWidth, totalWidth), height: frontBand });
    let rightX = living ? livingWidth : 0;
    if (kitchen) layout.push({ ...kitchen, x: rightX, y: 0, width: Math.max(8, Math.round(totalWidth * 0.28)), height: Math.max(7, Math.round(frontBand * 0.62)) });
    if (dining) layout.push({ ...dining, x: rightX, y: kitchen ? kitchen.height : 0, width: Math.max(8, Math.round(totalWidth * 0.22)), height: Math.max(6, frontBand - (kitchen ? kitchen.height : 0)) });
    bathrooms.forEach((bath, index) => {
      const bx = totalWidth - Math.max(6, Math.round(totalWidth * 0.16));
      const bw = Math.max(5, Math.round(totalWidth * 0.16));
      const bh = Math.max(6, Math.round((totalHeight - frontBand) / Math.max(1, bathrooms.length)));
      layout.push({ ...bath, x: bx, y: frontBand + index * bh, width: bw, height: bh });
    });
    if (bedrooms[0]) layout.push({ ...bedrooms[0], x: 0, y: frontBand, width: Math.max(10, Math.round(totalWidth * 0.5)), height: totalHeight - frontBand });
    if (bedrooms[1]) layout.push({ ...bedrooms[1], x: Math.max(10, Math.round(totalWidth * 0.5)), y: frontBand, width: totalWidth - Math.max(10, Math.round(totalWidth * 0.5)) - (bathrooms.length ? Math.max(5, Math.round(totalWidth * 0.16)) : 0), height: totalHeight - frontBand });
    const placedIds = new Set(layout.map((room) => room.id));
    typed.filter((room) => !placedIds.has(room.id)).forEach((room) => layout.push(room));
    return fitRoomsInGrid(layout, totalWidth, totalHeight);
  }

  if (normalizedCategory === "office") {
    const reception = typed.find((room) => room.type === "reception");
    const workspace = typed.find((room) => room.type === "workspace") || typed[0];
    const meeting = typed.find((room) => room.type === "meeting");
    const pantry = typed.find((room) => room.type === "pantry");
    const storage = typed.find((room) => room.type === "storage");
    const layout = [];
    const leftWidth = reception ? Math.max(8, Math.round(totalWidth * 0.22)) : 0;
    const rightWidth = meeting ? Math.max(9, Math.round(totalWidth * 0.26)) : 0;
    if (reception) layout.push({ ...reception, x: 0, y: 0, width: leftWidth, height: Math.max(8, Math.round(totalHeight * 0.3)) });
    if (workspace) layout.push({ ...workspace, x: leftWidth, y: 0, width: totalWidth - leftWidth - rightWidth, height: totalHeight });
    if (meeting) layout.push({ ...meeting, x: totalWidth - rightWidth, y: 0, width: rightWidth, height: Math.max(8, Math.round(totalHeight * 0.42)) });
    if (pantry) layout.push({ ...pantry, x: totalWidth - rightWidth, y: meeting ? meeting.height : 0, width: rightWidth, height: Math.max(5, Math.round(totalHeight * 0.18)) });
    if (storage) layout.push({ ...storage, x: 0, y: reception ? reception.height : totalHeight - 6, width: leftWidth || 8, height: Math.max(5, totalHeight - (reception ? reception.height : totalHeight - 6)) });
    const placedIds = new Set(layout.map((room) => room.id));
    typed.filter((room) => !placedIds.has(room.id)).forEach((room) => layout.push(room));
    return fitRoomsInGrid(layout, totalWidth, totalHeight);
  }
  ...
  if (normalizedCategory === "cafe") {
    const seating = typed.find((room) => room.type === "seating") || typed[0];
    const counter = typed.find((room) => room.type === "counter");
    const prep = typed.find((room) => room.type === "prep");
    const display = typed.find((room) => room.type === "display");
    const layout = [];
    const serviceWidth = Math.max(8, Math.round(totalWidth * 0.28));
    if (seating) layout.push({ ...seating, x: 0, y: 0, width: totalWidth - serviceWidth, height: totalHeight });
    if (counter) layout.push({ ...counter, x: totalWidth - serviceWidth, y: 0, width: serviceWidth, height: Math.max(6, Math.round(totalHeight * 0.32)) });
    if (prep) layout.push({ ...prep, x: totalWidth - serviceWidth, y: counter ? counter.height : 0, width: serviceWidth, height: Math.max(6, Math.round(totalHeight * 0.38)) });
    if (display) layout.push({ ...display, x: totalWidth - serviceWidth, y: totalHeight - Math.max(5, Math.round(totalHeight * 0.18)), width: serviceWidth, height: Math.max(5, Math.round(totalHeight * 0.18)) });
    const placedIds = new Set(layout.map((room) => room.id));
    typed.filter((room) => !placedIds.has(room.id)).forEach((room) => layout.push(room));
    return fitRoomsInGrid(layout, totalWidth, totalHeight);
  }

  if (normalizedCategory === "public toilet") {
    const wash = typed.find((room) => room.type === "wash");
    const others = typed.filter((room) => room.id !== wash?.id);
    const layout = [];
    const washHeight = wash ? Math.max(4, Math.round(totalHeight * 0.28)) : 0;
    if (wash) layout.push({ ...wash, x: 0, y: 0, width: totalWidth, height: washHeight });
    if (others[0]) layout.push({ ...others[0], x: 0, y: washHeight, width: Math.round(totalWidth * 0.55), height: totalHeight - washHeight });
    if (others[1]) layout.push({ ...others[1], x: Math.round(totalWidth * 0.55), y: washHeight, width: totalWidth - Math.round(totalWidth * 0.55), height: totalHeight - washHeight });
    return fitRoomsInGrid(layout.concat(others.slice(2)), totalWidth, totalHeight);
  }

  return fitRoomsInGrid(typed, totalWidth, totalHeight);
}

function getExternalWallsForRoom(room, totalWidth, totalHeight) {
  const walls = [];
  const x = Number(room.x) || 0;
  const y = Number(room.y) || 0;
  const width = Number(room.width) || 0;
  const height = Number(room.height) || 0;
  if (Math.abs(y) < 0.05) walls.push("top");
  if (Math.abs(x) < 0.05) walls.push("left");
  if (Math.abs((x + width) - totalWidth) < 0.05) walls.push("right");
  if (Math.abs((y + height) - totalHeight) < 0.05) walls.push("bottom");
  return walls;
}

function inferRotationForWall(wall) {
  if (wall === "top" || wall === "bottom") return 0;
  if (wall === "left") return 90;
  if (wall === "right") return 270;
  return 0;
}

function createFurnitureByTerms(category, terms, overrides = {}) {
  const preset = getFurnitureOptionsForCategory(category).find((item) => terms.some((term) => String(item.type || "").toLowerCase().includes(term)));
  return createFurnitureFromPreset(preset, category, overrides);
}

function basicFurnitureOverlap(a, b) {
  const ax1 = Number(a.x) || 0;
  const ay1 = Number(a.y) || 0;
  const ax2 = ax1 + (Number(a.width) || 0);
  const ay2 = ay1 + (Number(a.depth) || 0);
  const bx1 = Number(b.x) || 0;
  const by1 = Number(b.y) || 0;
  const bx2 = bx1 + (Number(b.width) || 0);
  const by2 = by1 + (Number(b.depth) || 0);
  return ax1 < bx2 && ax2 > bx1 && ay1 < by2 && ay2 > by1;
}

function reduceFurnitureCollisions(items, room) {
  const adjusted = [];
  (Array.isArray(items) ? items : []).forEach((item) => {
    let next = { ...item };
    for (let attempt = 0; attempt < 12; attempt += 1) {
      if (!adjusted.some((existing) => basicFurnitureOverlap(existing, next))) break;
      next = { ...next, x: (Number(next.x) || 0) + 0.8 };
      next = normalizeFurniture(next, room);
    }
    adjusted.push(normalizeFurniture(next, room));
  });
  return adjusted;
}

function autoPlaceFurnitureForRoom(room, category, prompt = "") {
  const normalizedCategory = normalizePlanningCategory(category, "office");
  const type = room.type || getRoomTypeFromName(room.name, normalizedCategory);
  const width = Number(room.width) || 10;
  const height = Number(room.height) || 10;
  const furniture = [];
  const add = (item) => { if (item) furniture.push(item); };

  if (normalizedCategory === "house") {
    if (type === "bedroom") {
      add(createFurnitureByTerms("house", ["bed"], { x: 1, y: 1, rotation: 0 }));
      add(createFurnitureByTerms("house", ["wardrobe"], { x: Math.max(1, width - 5.4), y: 1, rotation: 0 }));
    } else if (type === "living") {
      add(createFurnitureByTerms("house", ["sofa"], { x: 1, y: Math.max(1, height - 4), rotation: 0 }));
      add(createFurnitureByTerms("house", ["center table"], { x: Math.max(2, width / 2 - 2), y: Math.max(2, height / 2 - 1), rotation: 0 }));
      if (width >= 14 && height >= 10) add(createFurnitureByTerms("house", ["dining table"], { x: Math.max(2, width - 8), y: 1.5, rotation: 0 }));
    } else if (type === "kitchen") {
      add(createFurnitureByTerms("house", ["kitchen slab"], { attachedWall: "bottom", slabLength: Math.max(5, width - 2), slabDepth: 2, offset: 0 }));
      add(createFurnitureByTerms("house", ["stove"], { x: Math.max(1, width - 4), y: Math.max(1, height - 2.6), rotation: 0 }));
      add(createFurnitureByTerms("house", ["sink"], { x: 1, y: Math.max(1, height - 2.6), rotation: 0 }));
    }
  }

  if (normalizedCategory === "office") {
    if (type === "workspace") {
      const deskPreset = getFurnitureOptionsForCategory("office").find((item) => String(item.type).toLowerCase().includes("workstation"));
      const chairPreset = getFurnitureOptionsForCategory("office").find((item) => String(item.type).toLowerCase().includes("chair"));
      ...
      for (let row = 0; row < rows; row += 1) {
        for (let col = 0; col < columns; col += 1) {
          const x = 1 + col * 5.5;
          const y = 1 + row * 4;
          add(createFurnitureFromPreset(deskPreset, "office", { x, y, rotation: 0 }));
          add(createFurnitureFromPreset(chairPreset, "office", { x: x + 1.2, y: Math.min(height - 2, y + 2.2), rotation: 180 }));
          deskIndex += 1;
          if (deskIndex >= 12) break;
        }
        if (deskIndex >= 12) break;
      }
      add(createFurnitureByTerms("office", ["storage cabinet"], { x: Math.max(1, width - 4.5), y: 1, rotation: 0 }));
    } else if (type === "reception") {
      add(createFurnitureByTerms("office", ["reception"], { x: 1, y: Math.max(1, height / 2 - 1.5), rotation: 0 }));
      add(createFurnitureByTerms("office", ["chair"], { x: width - 3, y: Math.max(1, height / 2 - 1), rotation: 180 }));
    } else if (type === "meeting") {
      add(createFurnitureByTerms("office", ["conference table"], { x: Math.max(1, width / 2 - 4), y: Math.max(1, height / 2 - 2), rotation: 0 }));
    }
  }

  if (normalizedCategory === "cafe") {
    if (type === "seating") {
      const fourSeat = getFurnitureOptionsForCategory("cafe").find((item) => String(item.type).toLowerCase().includes("4-seater"));
      const chair = getFurnitureOptionsForCategory("cafe").find((item) => String(item.type).toLowerCase() === "chair");
      const columns = Math.max(1, Math.floor((width - 2) / 6));
      const rows = Math.max(1, Math.floor((height - 2) / 6));
      for (let row = 0; row < rows; row += 1) {
        for (let col = 0; col < columns; col += 1) {
          const x = 1 + col * 6;
          const y = 1 + row * 6;
          add(createFurnitureFromPreset(fourSeat, "cafe", { x, y, rotation: 0 }));
          add(createFurnitureFromPreset(chair, "cafe", { x: x - 0.2, y: y + 1.1, rotation: 90 }));
          add(createFurnitureFromPreset(chair, "cafe", { x: x + 3.6, y: y + 1.1, rotation: 270 }));
        }
      }
    } else if (type === "counter") {
      add(createFurnitureByTerms("cafe", ["service counter"], { x: 0.8, y: Math.max(1, height / 2 - 1.25), rotation: 0 }));
      if (/display/.test(String(prompt || "").toLowerCase())) add(createFurnitureByTerms("cafe", ["display"], { x: Math.max(1, width - 4.2), y: 1, rotation: 0 }));
    } else if (type === "prep") {
      add(createFurnitureByTerms("cafe", ["display"], { x: 1, y: 1, rotation: 0 }));
      add(createFurnitureByTerms("house", ["kitchen slab"], { attachedWall: "bottom", slabLength: Math.max(5, width - 2), offset: 0 }));
    }
  }

  if (normalizedCategory === "public toilet") {
    if (type === "stalls" || type === "toilet") {
      const wc = getFurnitureOptionsForCategory("public toilet").find((item) => String(item.type).toLowerCase().includes("toilet seat"));
      const urinal = getFurnitureOptionsForCategory("public toilet").find((item) => String(item.type).toLowerCase().includes("urinal"));
      const partition = getFurnitureOptionsForCategory("public toilet").find((item) => String(item.type).toLowerCase().includes("partition"));
      const count = Math.max(2, Math.floor((width - 1) / 3));
      for (let i = 0; i < count; i += 1) {
        add(createFurnitureFromPreset(wc, "public toilet", { x: 0.8 + i * 2.8, y: Math.max(1, height - 4.5), rotation: 0 }));
        if (i < count - 1) add(createFurnitureFromPreset(partition, "public toilet", { x: 2.4 + i * 2.8, y: 1, rotation: 90, width: 0.3, depth: Math.max(4, height - 2), height: 6.5 }));
      }
      if (width >= 8) add(createFurnitureFromPreset(urinal, "public toilet", { x: Math.max(1, width - 2.4), y: 1, rotation: 90 }));
    } else if (type === "wash") {
      add(createFurnitureByTerms("public toilet", ["wash basin"], { x: 1, y: 1, rotation: 0 }));
      add(createFurnitureByTerms("public toilet", ["mirror"], { x: 3.5, y: 0.4, rotation: 0 }));
    }
  }

  if (normalizedCategory === "security cabin") {
    add(createFurnitureByTerms("security cabin", ["small desk"], { x: 1, y: Math.max(1, height - 3), rotation: 0 }));
    add(createFurnitureByTerms("security cabin", ["guard chair"], { x: 2.5, y: Math.max(1, height - 4.5), rotation: 180 }));
    add(createFurnitureByTerms("security cabin", ["cctv"], { x: Math.max(1, width - 4), y: 1, rotation: 0 }));
    add(createFurnitureByTerms("security cabin", ["storage shelf"], { x: Math.max(1, width - 3.5), y: Math.max(1, height - 2.5), rotation: 0 }));
  }

  if (normalizedCategory === "storage") {
    add(createFurnitureByTerms("storage", ["storage rack"], { x: 1, y: 1, rotation: 0 }));
    add(createFurnitureByTerms("storage", ["heavy duty"], { x: Math.max(1, width - 9), y: 1, rotation: 0 }));
    add(createFurnitureByTerms("storage", ["utility table"], { x: Math.max(1, width / 2 - 2.5), y: Math.max(1, height - 4), rotation: 0 }));
  }

  return reduceFurnitureCollisions(furniture.filter(Boolean), room);
}

function autoPlaceDoorsAndWindows(rooms, totalWidth, totalHeight, category, prompt = "") {
  const normalizedCategory = normalizePlanningCategory(category, "office");
  const commonTypes = new Set(["living", "workspace", "seating", "reception", "cabin", "storage"]);
  const commonRoom = rooms.find((room) => commonTypes.has(room.type || getRoomTypeFromName(room.name, normalizedCategory))) || rooms[0];
  const lower = String(prompt || "").toLowerCase();
  const wantsMoreWindows = /airy|sunlight|windows|premium|luxury|bright/.test(lower);

  return rooms.map((room) => {
    const type = room.type || getRoomTypeFromName(room.name, normalizedCategory);
    const externals = getExternalWallsForRoom(room, totalWidth, totalHeight);
    let doors = [];
    if (room.id === commonRoom?.id) {
      const outerWall = externals.includes("bottom") ? "bottom" : externals[0] || "bottom";
      const wallLength = getWallLength(room, outerWall);
      const doorWidth = clamp(Math.min(4, Math.max(3, wallLength * 0.22)), 2.5, Math.max(2.5, wallLength - 0.5));
      doors.push({ wall: outerWall, offset: Math.max(0.4, (wallLength - doorWidth) / 2), width: doorWidth, height: DEFAULT_DOOR_HEIGHT });
    } else {
      const shared = rooms
        .filter((other) => other.id !== room.id)
        .map((other) => ({ other, boundary: getSharedBoundary(room, other) }))
        .find((entry) => entry.boundary && (entry.other.id === commonRoom?.id || commonTypes.has(entry.other.type || getRoomTypeFromName(entry.other.name, normalizedCategory))));
      if (shared?.boundary) {
        const span = Math.max(2.8, shared.boundary.end - shared.boundary.start);
        const doorWidth = clamp(Math.min(3.2, span - 0.4), 2.5, span - 0.1);
        const offset = Math.max(0.2, shared.boundary.start - (shared.boundary.wallA === "left" || shared.boundary.wallA === "right" ? Number(room.y) || 0 : Number(room.x) || 0) + (span - doorWidth) / 2);
        doors.push({ wall: shared.boundary.wallA, offset, width: doorWidth, height: DEFAULT_DOOR_HEIGHT });
      } else {
        const fallbackWall = externals[0] || "left";
        const wallLength = getWallLength(room, fallbackWall);
        doors.push({ wall: fallbackWall, offset: Math.max(0.4, (wallLength - DEFAULT_DOOR_WIDTH) / 2), width: clamp(DEFAULT_DOOR_WIDTH, 2.5, wallLength - 0.2), height: DEFAULT_DOOR_HEIGHT });
      }
    }

    let windows = [];
    const preferredWalls = externals.length ? externals : ["top"];
    const desiredCount = type === "bathroom" || type === "wash" ? 1 : wantsMoreWindows ? 2 : 1;
    preferredWalls.slice(0, desiredCount).forEach((wall, index) => {
      const wallLength = getWallLength(room, wall);
      const width = clamp((type === "bathroom" || type === "wash") ? 2.2 : Math.min(5, wallLength * 0.32), 2, Math.max(2, wallLength - 0.4));
      const offset = desiredCount === 1 ? Math.max(0.2, (wallLength - width) / 2) : Math.max(0.2, index === 0 ? 0.6 : wallLength - width - 0.6);
      windows.push({ wall, offset, width, height: type === "bathroom" || type === "wash" ? 2.2 : DEFAULT_WINDOW_HEIGHT, sillHeight: type === "bathroom" || type === "wash" ? 4 : DEFAULT_WINDOW_SILL_HEIGHT });
    });
...
function improveRoomAdjacency(rooms, category, totalWidth, totalHeight, prompt = "") {
  return placeRoomsByCategory(rooms, category, totalWidth, totalHeight, prompt);
}

function validateAndRepairAIPlan(plan, currentState = {}, prompt = "") {
  const rawCategory = normalizePlanningCategory(plan?.selectedCategory, inferCategoryFromPrompt(prompt, normalizePlanningCategory(currentState?.selectedCategory, "office")));
  const dims = getDefaultPlanDimensionsByCategory(rawCategory, prompt, currentState);
  const totalWidth = clamp(Number(plan?.totalWidth) || dims.totalWidth, 10, 200);
  const totalHeight = clamp(Number(plan?.totalHeight) || dims.totalHeight, 10, 200);
  const rawRooms = Array.isArray(plan?.rooms) ? plan.rooms : [];
  const sourceRooms = rawRooms.length ? rawRooms : (generateCategorySeedLayout(rawCategory, totalWidth, totalHeight, prompt).rooms || []);

  const sanitizedRooms = sourceRooms.map((room, index) => {
    const type = room?.type || getRoomTypeFromName(room?.name, rawCategory);
    const min = getMinimumRoomSize(type, rawCategory);
    return buildPlanningRoom(
      index,
      room?.name || `${getFriendlyCategoryName(type)} ${index + 1}`,
      type,
      Math.max(min.width, Number(room?.width) || min.width),
      Math.max(min.height, Number(room?.height) || min.height),
      rawCategory,
      {
        ...room,
  ...
  let arrangedRooms = improveRoomAdjacency(sanitizedRooms, rawCategory, totalWidth, totalHeight, prompt);
  arrangedRooms = arrangedRooms.map((room, index) => {
    const fallbackFurniture = autoPlaceFurnitureForRoom(room, rawCategory, prompt);
    const incomingFurniture = Array.isArray(room.furniture) && room.furniture.length
      ? room.furniture.map((item) => createFurnitureFromPreset({
          type: item.type || `Furniture ${index + 1}`,
          width: Number(item.width) || 3,
          depth: Number(item.depth) || 2,
          height: Number(item.height) || 3,
          color: item.color || "#d4dde8",
          allowOutsideBuilding: Boolean(item.allowOutsideBuilding),
        }, rawCategory, {
          x: Number(item.x),
          y: Number(item.y),
          rotation: Number(item.rotation) || 0,
          attachedWall: item.attachedWall,
          slabLength: Number(item.slabLength),
          slabDepth: Number(item.slabDepth),
    ...
    return normalizeRoom({
      ...
      x: Number(room.x) || 0,
      y: Number(room.y) || 0,
      furniture: reduceFurnitureCollisions(incomingFurniture.length ? incomingFurniture : fallbackFurniture, room),
      doors: Array.isArray(room.doors) ? room.doors : [],
      windows: Array.isArray(room.windows) ? room.windows : [],
    }, totalWidth, totalHeight, Number(currentState?.roomHeight) || DEFAULT_ROOM_HEIGHT);
  });

  arrangedRooms = autoPlaceDoorsAndWindows(arrangedRooms, totalWidth, totalHeight, rawCategory, prompt).map((room) => normalizeRoom({
    ...room,
    furniture: reduceFurnitureCollisions(Array.isArray(room.furniture) && room.furniture.length ? room.furniture : autoPlaceFurnitureForRoom(room, rawCategory, prompt), room),
  }, totalWidth, totalHeight, Number(currentState?.roomHeight) || DEFAULT_ROOM_HEIGHT));

  if (!arrangedRooms.length) {
    const fallback = generateCategorySeedLayout(rawCategory, totalWidth, totalHeight, prompt);
    return validateAndRepairAIPlan(fallback, currentState, prompt);
  }

  return {
    planName: String(plan?.planName || `${getFriendlyCategoryName(rawCategory)} Layout`),
    selectedCategory: rawCategory,
    totalWidth,
    totalHeight,
    responseText: String(plan?.responseText || `Created a practical ${getFriendlyCategoryName(rawCategory).toLowerCase()} layout.`),
    rooms: arrangedRooms,
  };
}

function buildPresetTemplate(kind, totalWidth, totalHeight, prompt = "") {
  const category = normalizePlanningCategory(kind, inferCategoryFromPrompt(prompt, "office"));
  return validateAndRepairAIPlan(generateCategorySeedLayout(category, totalWidth, totalHeight, prompt), { totalWidth, totalHeight, selectedCategory: category }, prompt);
}

function normalizeGeneratedRooms(rooms, totalWidth, totalHeight, category, prompt = "") {
  return validateAndRepairAIPlan({ selectedCategory: category, totalWidth, totalHeight, rooms }, { totalWidth, totalHeight, selectedCategory: category }, prompt).rooms;
}
...
function parseRuleBasedPlanCommand(prompt, currentState) {
  ...
  const inferredCategory = inferCategoryFromPrompt(text, normalizePlanningCategory(currentState?.selectedCategory, "office"));
  const { totalWidth, totalHeight } = getDefaultPlanDimensionsByCategory(inferredCategory, text, currentState);

  const directPlanIntent = /(create|make|design|plan|generate|build|layout)/.test(lower) || /(2\s*bhk|1\s*bhk|studio|cafe|office|toilet|restroom|security cabin|guard room|warehouse|storage)/.test(lower);
  if (directPlanIntent) {
    return buildPresetTemplate(inferredCategory, totalWidth, totalHeight, text);
  }

  const addMatch = lower.match(/add\s+(\d+)\s+rooms?/);
  if (addMatch) {
    const category = normalizePlanningCategory(currentState.selectedCategory, inferredCategory);
    const roomCount = Math.max(1, Number(addMatch[1]) || 1);
    const generatedRooms = Array.from({ length: roomCount }, (_, i) => buildPlanningRoom(i, `Room ${i + 1}`, "room", 10, 10, category));
    return validateAndRepairAIPlan({
      planName: currentState.planName || `${getFriendlyCategoryName(category)} Layout`,
...
function sanitizeOpenAIPlanResponse(aiResponse, currentState, userPrompt = "") {
  if (!aiResponse || typeof aiResponse !== "object") return null;
  const repairedBase = validateAndRepairAIPlan({
    planName: aiResponse.planName,
    selectedCategory: aiResponse.selectedCategory,
    totalWidth: aiResponse.totalWidth,
    totalHeight: aiResponse.totalHeight,
    responseText: aiResponse.responseText,
    rooms: Array.isArray(aiResponse.rooms) ? aiResponse.rooms : [],
  }, currentState, userPrompt);

  const variants = Array.isArray(aiResponse.variants)
    ? aiResponse.variants
        .map((variant, index) => {
          const repaired = validateAndRepairAIPlan({
            planName: repairedBase.planName,
            selectedCategory: aiResponse.selectedCategory || repairedBase.selectedCategory,
            totalWidth: aiResponse.totalWidth || repairedBase.totalWidth,
            totalHeight: aiResponse.totalHeight || repairedBase.totalHeight,
            responseText: variant?.rationale || repairedBase.responseText,
            rooms: Array.isArray(variant?.rooms) ? variant.rooms : repairedBase.rooms,
          }, currentState, userPrompt);
          return {
            ...repaired,
            id: String(variant?.id || `option-${index + 1}`),
            label: String(variant?.label || `Option ${String.fromCharCode(65 + index)}`),
            rationale: String(variant?.rationale || repaired.responseText),
          };
        })
        .filter(Boolean)
    : [];
  ...
  return {
    ...repairedBase,
    responseText: aiResponse.responseText || repairedBase.responseText,
    variants,
  };
}
...
async function generatePlanFromOpenAI(apiKey, userPrompt, currentState) {
  ...
  const inferredCategory = inferCategoryFromPrompt(userPrompt, normalizePlanningCategory(currentState?.selectedCategory, "office"));
  const dims = getDefaultPlanDimensionsByCategory(inferredCategory, userPrompt, currentState);
  const seed = generateCategorySeedLayout(inferredCategory, dims.totalWidth, dims.totalHeight, userPrompt);
  const response = await fetch("https://api.openai.com/v1/responses", {
    ...
    body: JSON.stringify({
      ...
      input: [
        {
          role: "system",
          content: `You are a floor-planning expert helping a React floor planner app. Return structured JSON only. Interpret layman requests like "create a 2BHK", "create a cafe", "office for 10 people", "make it airy", or "add more windows". Use practical room programs, realistic room sizes, usable circulation, sensible furniture rotation, windows on external walls where possible, and doors that do not obviously open into furniture. If the request is vague, infer practical defaults. Prefer 2 or 3 variants when useful.`,
        },
        {
          role: "user",
          content: `Current app context:\n${JSON.stringify({
            selectedCategory: currentState?.selectedCategory,
            totalWidth: currentState?.totalWidth,
            totalHeight: currentState?.totalHeight,
            roomHeight: currentState?.roomHeight,
          }, null, 2)}\n\nUser request:\n${userPrompt}\n\nInferred category: ${inferredCategory}\nSuggested seed layout:\n${JSON.stringify(seed, null, 2)}`,
        },
      ],
              ...
              properties: {
                ...
                doors: {
                  ...
                  items: {
                    ...
                    required: ["wall", "offset", "width", "height"],
                ...
                windows: {
                  ...
                  items: {
                    ...
                    required: ["wall", "offset", "width", "height", "sillHeight"],
                    additionalProperties: false
                  }
                },
                furniture: {
                  ...
                  items: {
                    ...
                    properties: {
                      ...
                      color: { type: "string" },
                      attachedWall: { type: "string" },
                      slabLength: { type: "number" },
                      slabDepth: { type: "number" },
                      offset: { type: "number" }
                    },
  ...
  return sanitizeOpenAIPlanResponse(JSON.parse(rawText), currentState, userPrompt);
}
...
function generateLayoutVariants(basePlan, prompt = "") {
  const normalizedBase = validateAndRepairAIPlan(basePlan, basePlan, prompt);
  const aiVariants = Array.isArray(basePlan?.variants) ? basePlan.variants : [];
  if (aiVariants.length >= 2) {
    return aiVariants.slice(0, 3).map((variant, index) => ({
      ...validateAndRepairAIPlan({ ...normalizedBase, ...variant, rooms: variant.rooms }, normalizedBase, prompt),
      id: variant.id || `option-${index + 1}`,
      label: variant.label || `Option ${String.fromCharCode(65 + index)}`,
      rationale: variant.rationale || variant.responseText || normalizedBase.responseText,
    }));
  }

  const styles = [
    { id: "standard", label: "Option A — Balanced", rationale: "Balanced room sizes with practical access and everyday usability.", style: "standard" },
    { id: "compact", label: "Option B — Efficient", rationale: "A slightly tighter plan with shorter walking paths and efficient space usage.", style: "compact" },
    { id: "airy", label: "Option C — Airy", rationale: "A more open-feeling plan with stronger common areas and extra windows where practical.", style: "airy" },
  ];

  return styles.map((config) => {
    const seed = generateCategorySeedLayout(normalizedBase.selectedCategory, normalizedBase.totalWidth, normalizedBase.totalHeight, prompt, config.style);
    const variantRooms = seed.rooms && seed.rooms.length ? seed.rooms : normalizedBase.rooms;
    const repaired = validateAndRepairAIPlan({
      ...normalizedBase,
      rooms: variantRooms,
      responseText: config.rationale,
...
export default function App() {
  ...
  const handleGenerateLayout = async (prompt) => {
    ...
    try {
      ...
      const variants = generateLayoutVariants(basePlan, prompt);
  ...
  const handleChatSubmit = async (event) => {
    ...
    try {
      ...
      appendChatMessage("assistant", "I understood the request only partially. Please try a more specific command like 'Create a 2BHK in 40 by 30 feet'.");
    } catch (error) {
